...
on_client_connected(#{client_id := ClientId}, ConnAck, ConnAttrs, _Env) ->
    ...
    %%ekaf:produce_async_batched(?TEST_TOPIC, Json),批量发送，不建议使用，我发现kafka接收会滞后
    ekaf:produce_async(?TEST_TOPIC, Json),
    ekaf:produce_async(?TEST_TOPIC, {<<"mykey_1">>, Json}).
...
on_message_publish(Message, _Env) ->
    ...
    ekaf:produce_async_batched(?TEST_TOPIC, Json),
